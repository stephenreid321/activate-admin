      ...
      params[:qk].each_with_index { |fieldname,i|
        ...
        if options[:type] == :lookup              
          ...
          if matchable_regex.include?(options[:type]) 
            ...
            query << {:id.send(b) => collection_model.where(:coordinates => { "$geoWithin" => { "$centerSphere" => [Geocoder.coordinates(q.split(':')[0].strip).reverse, (q.split(':')[1].strip || 20).to_i / 3963.1676 ]}}).pluck(collection_key)}
